...
export class Migration20220221094915 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `nftstate` (`uuid` text not null, `token_id` text not null, `base16` text not null, `url` text null, `meta` json null, `nft_id` integer not null, constraint `nftstate_nft_id_foreign` foreign key(`nft_id`) references `token`(`id`) on update cascade, primary key (`uuid`));');
